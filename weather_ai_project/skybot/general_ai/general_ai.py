# skybot/general_ai/general_ai.py
...
class GeneralAI:
    ...
    def heuristic_decision(self, fused: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 50 <= threat < 80:
            return {
                ...
                "reason": "Moderate threat, need more info",
                "confidence": 0.55
            }
    ...
    def ai_interpretation(self, fused: Dict[str, Any], heuristic: Dict[str, Any]) -> Dict[str, Any]:
        system_prompt = (
            ...
            "You must decide whether to send an alert, request more data, or stay idle."
        )

        user_prompt = (
            f"Fused data: {fused}\n"
            f"Heuristic suggestion: {heuristic}\n\n"
            "If the heuristic seems reasonable, keep the same action but you may adjust the reason and confidence. "
            "Respond in JSON with keys: action, reason, confidence (0-1)."
        )
